    ...
    Pass
    {
      ...
      inline half3 Neon(half3 pixel, float2 uv)
      {
        ...
        for (int i = -_Radius ; i <= _Radius; i++)
        {
          ...
          {
            pixel = pow(max(SAMPLE_MAIN_LOD(uv + float2(i, j) * _MainTex_TexelSize.xy).rgb, 0.0), (float3)2.2);
        ...
        pixel = HSV2RGBSmooth(float3(angle, 1.0, pow(Luminance601(derivative) * 30.0 * _Strength, 3.0) * 5.0));
        pixel = pow(max(pixel, 0.0), (float3)(1.0 / 2.2));
      ...
      half4 ArtisticFrag(const VertexOutput input) : SV_Target
      {
        ...
        pixel.rgb = MixFog(pixel.rgb, input.fogFactor);
#if ARTISTIC_DEMO
