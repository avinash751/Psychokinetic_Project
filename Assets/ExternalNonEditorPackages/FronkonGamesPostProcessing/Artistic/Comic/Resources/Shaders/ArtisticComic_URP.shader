...
Shader "Hidden/Fronkon Games/Artistic/Comic URP"
{
  ...
  SubShader
  {
    ...
    Pass
    {
      ...
      {
        float f = dot(SAMPLE_MAIN(uv), (float3)1.0);
        float2 grad = float2(dot(SAMPLE_MAIN(uv + _Edge * float2(_MainTex_TexelSize.x, 0.0)).rgb, (float3)1.0),
                             dot(SAMPLE_MAIN(uv + _Edge * float2(0.0, _MainTex_TexelSize.y)).rgb, (float3)1.0)) - f;
