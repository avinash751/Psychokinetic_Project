...
VertexOutput ArtisticVert(VertexInput input)
{
  ...
  float4 sp = ComputeScreenPos(output.positionCS);
  output.screenPos = _ScreenParams.xy * (sp.xy / sp.w) - _ScreenParams.xy * 0.5;
...
{
  const half separatorWidth = 6.0 * _MainTex_TexelSize.x;
