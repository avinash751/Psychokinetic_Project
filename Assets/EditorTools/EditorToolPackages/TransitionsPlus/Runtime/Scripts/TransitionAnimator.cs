...
// inside Editor, the capure/toggle canvas from UpdateMaterialProperties() is delayed on frame to avoid OnValidate errors so we force the refresh here, to ensure camera image is present on frame 0
            ...
            if (Application.isPlaying) {
                if (autoPlay) {
                    InvokeUnscaled(StartTransition, playDelay);
                }
            }
        }

        public void InvokeUnscaled(Action method, float delay)
        {
            StartCoroutine(InvokeUnscaledCoroutine(method, delay));
        }

        private IEnumerator InvokeUnscaledCoroutine(Action method, float delay)
        {
