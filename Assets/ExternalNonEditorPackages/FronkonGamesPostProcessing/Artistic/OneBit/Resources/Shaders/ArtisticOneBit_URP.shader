  ...
  SubShader
  {
    ...
    Pass
    {
      ...
      float Mask(float2 p)
      {
        ...
        return Poly(pow(abs(f), 150.0), f * f, f, 1.0, 0.0, 1.3);
      }
      ...
      half4 ArtisticFrag(const VertexOutput input) : SV_Target
      {
        ...
        pixel.rgb = SAMPLE_TEXTURE2D(_GradientTex, sampler_LinearClamp, half2(lum, 0.5));
...
#endif
        pixel.r = floor((_RedCount - 1.0f) * pixel.r + 0.5) / (_RedCount - 1.0f);
        pixel.g = floor((_GreenCount - 1.0f) * pixel.g + 0.5) / (_GreenCount - 1.0f);
        pixel.b = floor((_BlueCount - 1.0f) * pixel.b + 0.5) / (_BlueCount - 1.0f);
