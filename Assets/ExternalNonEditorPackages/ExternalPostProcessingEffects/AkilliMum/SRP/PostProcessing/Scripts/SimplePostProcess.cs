    ...
    public class SimplePostProcess : ScriptableRendererFeature
    {
        class CustomRenderPass : ScriptableRenderPass
        {
            ...
            public override void Configure(CommandBuffer cmd, RenderTextureDescriptor cameraTextureDescriptor)
            {
                ...
                //Debug.Log("configure blur called..");

                _tmpId1 = Shader.PropertyToID("tmpRT1");
                cmd.GetTemporaryRT(_tmpId1, width, height, 0, FilterMode.Trilinear, cameraTextureDescriptor.graphicsFormat);

                _tmpRt1 = new RenderTargetIdentifier(_tmpId1);

                //_tmpRt1 = RTHandles.Alloc(_tmpId1);
            ...
            public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
            {
                ...
                using (new ProfilingScope(cmd, new ProfilingSampler(Tag)))
                {
                    ...
                    {
                        cmd.Blit(renderingData.cameraData.renderer.cameraColorTargetHandle, _tmpRt1, _activeMat);
                        cmd.Blit(_tmpRt1, renderingData.cameraData.renderer.cameraColorTargetHandle);
                    }
                }
